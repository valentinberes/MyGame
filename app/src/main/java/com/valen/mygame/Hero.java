package com.valen.mygame;

import android.content.Context;
import android.graphics.Bitmap;
import android.graphics.BitmapFactory;
import android.opengl.GLES32;
import android.opengl.GLUtils;
import android.util.Log;

import java.io.IOException;
import java.io.InputStream;
import java.nio.ByteBuffer;
import java.nio.ByteOrder;
import java.nio.FloatBuffer;
import java.nio.ShortBuffer;

/**
 * Created by valen on 11/05/2017.
 * Project: MyGame
 */

public class Hero {
	static final int COORDS_PER_TEXTURE = 2;
	static final int COORDS_PER_VERTEX = 3;
	// fields
	private static final String TAG = "Hero";
	public static int textureStride = COORDS_PER_TEXTURE * 4;
	static float squareCoords[] = {
			-0.25f, 0.25f, 0.0f,   // top left
			-0.25f, -0.25f, 0.0f,   // bottom left
			0.25f, -0.25f, 0.0f,   // bottom right
			0.25f, 0.25f, 0.0f}; // top right
	private final short drawOrder[] = {0, 1, 2, 0, 2, 3};
	private final String vertexShaderCode =
			"uniform mat4 uMVPMatrix;" +
					"attribute vec4 vPosition;" +
					"attribute vec2 TexCoordIn;" +
					"varying vec2 TexCoordOut;" +
					"void main() {" +
					"  gl_Position = uMVPMatrix * vPosition;" +
					"  TexCoordOut = TexCoordIn;" +
					"}";
	private final String fragmentShaderCode =
			"precision mediump float;" +
					"uniform vec4 vColor;" +
					"uniform sampler2D TexCoordIn;" +
					"uniform float posX;" +
					"uniform float posY;" +
					"varying vec2 TexCoordOut;" +
					"void main() {" +
					" gl_FragColor = texture2D(TexCoordIn, vec2(TexCoordOut.x + posX ,TexCoordOut.y + posY ));" +
					"}";
	private final FloatBuffer vertexBuffer;
	private final ShortBuffer drawListBuffer;
	private final FloatBuffer textureBuffer;
	private final int mProgram;
	private final int vertexStride = COORDS_PER_VERTEX * 4;
	private float texture[] = {
			0f, 0.25f,
			0f, 0f,
			0.25f, 0f,
			0.25f, 0.25f,
	};
	private int[] textures = new int[1];
	private int mPositionHandle;
	private int mMVPMatrixHandle;

	public Hero() {
		ByteBuffer bb = ByteBuffer.allocateDirect(squareCoords.length * 4);
		bb.order(ByteOrder.nativeOrder());
		vertexBuffer = bb.asFloatBuffer();
		vertexBuffer.put(squareCoords);
		vertexBuffer.position(0);
		bb = ByteBuffer.allocateDirect(texture.length * 4);
		bb.order(ByteOrder.nativeOrder());
		textureBuffer = bb.asFloatBuffer();
		textureBuffer.put(texture);
		textureBuffer.position(0);
		ByteBuffer dlb = ByteBuffer.allocateDirect(drawOrder.length * 2);
		dlb.order(ByteOrder.nativeOrder());
		drawListBuffer = dlb.asShortBuffer();
		drawListBuffer.put(drawOrder);
		drawListBuffer.position(0);
		int vertexShader = GameRenderer.loadShader(GLES32.GL_VERTEX_SHADER, vertexShaderCode);
		int fragmentShader = GameRenderer.loadShader(GLES32.GL_FRAGMENT_SHADER, fragmentShaderCode);
		mProgram = GLES32.glCreateProgram();
		GLES32.glAttachShader(mProgram, vertexShader);
		GLES32.glAttachShader(mProgram, fragmentShader);
		GLES32.glLinkProgram(mProgram);
	}

	// methods
	public void loadTexture(int texture, Context context) {
		InputStream imagestream = context.getResources().openRawResource(texture);
		Bitmap bitmap = null;
		android.graphics.Matrix flip = new android.graphics.Matrix();
		flip.postScale(-1f, -1f);
		try {
			bitmap = BitmapFactory.decodeStream(imagestream);
		} catch (Exception e) {
			Log.e(TAG, "Exception: " + e.getMessage());
		} finally {
			try {
				imagestream.close();
				imagestream = null;
			} catch (IOException e) {
				Log.e(TAG, "IOException: " + e.getMessage());
			}
		}
		GLES32.glGenTextures(1, textures, 0);
		GLES32.glBindTexture(GLES32.GL_TEXTURE_2D, textures[0]);
		GLES32.glTexParameterf(GLES32.GL_TEXTURE_2D, GLES32.GL_TEXTURE_MIN_FILTER, GLES32.GL_LINEAR);
		GLES32.glTexParameterf(GLES32.GL_TEXTURE_2D, GLES32.GL_TEXTURE_MAG_FILTER, GLES32.GL_LINEAR);
		GLES32.glTexParameterf(GLES32.GL_TEXTURE_2D, GLES32.GL_TEXTURE_WRAP_S, GLES32.GL_CLAMP_TO_EDGE);
		GLES32.glTexParameterf(GLES32.GL_TEXTURE_2D, GLES32.GL_TEXTURE_WRAP_T, GLES32.GL_CLAMP_TO_EDGE);
		GLUtils.texImage2D(GLES32.GL_TEXTURE_2D, 0, bitmap, 0);
		bitmap.recycle();
	}

	public void draw(float[] mvpMatrix, float posX, float posY) {
		GLES32.glUseProgram(mProgram);
		mPositionHandle = GLES32.glGetAttribLocation(mProgram, "vPosition");
		GLES32.glEnableVertexAttribArray(mPositionHandle);
		int vsTextureCoord = GLES32.glGetAttribLocation(mProgram, "TexCoordIn");
		GLES32.glVertexAttribPointer(mPositionHandle, COORDS_PER_VERTEX, GLES32.GL_FLOAT, false, vertexStride, vertexBuffer);
		GLES32.glVertexAttribPointer(vsTextureCoord, COORDS_PER_TEXTURE, GLES32.GL_FLOAT, false, textureStride, textureBuffer);
		GLES32.glEnableVertexAttribArray(vsTextureCoord);
		GLES32.glActiveTexture(GLES32.GL_TEXTURE0);
		int fsTexture = GLES32.glGetUniformLocation(mProgram, "TexCoordOut");
		int fsPosX = GLES32.glGetUniformLocation(mProgram, "posX");
		int fsPosY = GLES32.glGetUniformLocation(mProgram, "posY");
		GLES32.glUniform1i(fsTexture, 0);
		GLES32.glUniform1f(fsPosX, posX);
		GLES32.glUniform1f(fsPosY, posY);
		mMVPMatrixHandle = GLES32.glGetUniformLocation(mProgram, "uMVPMatrix");
		GameRenderer.checkGlError("glGetUniformLocation");
		GLES32.glUniformMatrix4fv(mMVPMatrixHandle, 1, false, mvpMatrix, 0);
		GameRenderer.checkGlError("glUniformMatrix4fv");
		GLES32.glBindTexture(GLES32.GL_TEXTURE_2D, textures[0]);
		GLES32.glDrawElements(GLES32.GL_TRIANGLES, drawOrder.length, GLES32.GL_UNSIGNED_SHORT, drawListBuffer);
		GLES32.glDisableVertexAttribArray(mPositionHandle);
	}
}
